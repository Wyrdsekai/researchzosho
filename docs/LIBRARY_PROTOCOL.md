...
Investigations from `library_get` also carry `sections[]` (`{heading, chars}`, in order), `sources[]` as
rows `{n, locator, title, edition, published, fetched, language, same_as, tier, rule, why}` read from the
write-up's References (`fetched`: the shelves hold the page's text; `language`: of the notes the workers took
from it; `same_as`: the reference it is a copy of), `claims[]` (the findings the run produced, each as a finding
entry with its body and sources; `findings[]` stays the list of ids) and `open_questions[]`.
...
| `library_get` | `id`, `section?`, `offset?`, `max_chars?`, `patron?` | `entry` in full (see §2), with `chars` (the body's length). `id` is the FULL id as returned elsewhere (`F-0022-lead-paint-ban-1978-…`), never the bare serial. `section` returns one section of the body by its heading (case-insensitive; a prefix or a word of it will do), or `answer` for the write-up without the harness's own sections (Question, Sources, Caveats, Cite-check, Coverage check, Evidence, References, Languages, Web search, Source requests, Worker findings, Sources cited); `workers` is the worker findings. An unknown section is `invalid_args` naming the sections. `offset` and `max_chars` window the body (`offset`, `truncated` returned); `chars` is always the whole, so a caller can page |
...
| `library_add` | `path` xor `url`, `collection?`, `mode`=keep \| link \| survey, `register?`, `patron?` | material read in as it is (no model): for a folder `{kind: "folder", collection, seen, added, unchanged, changed, skipped, problems[], summary, next}` or, with `mode=survey`, `{files, bytes, text_bytes_if_kept, disk_free_bytes, by_type, summary}`; for a file or a url `{kind, raw, title, added: 1}`. `keep` copies the extracted text onto the shelves, `link` reads the file in place (a pointer, a hash and a size on the shelf; "not reachable now" when the drive is away). A `path` is accepted from the keeper only (the terminal, the library's own pages; `forbidden` otherwise); a `url` from any patron with write access, always kept. Route `/v1/add`. |
| `library_absorb` | one of `path` (keeper only), `url`, `text`; `title?`, `collection`=conversations, `verify?`, `limit`=25, `patron?` | a conversation with another assistant as a starting point: `{threads[] {title, raw, turns, questions_filed[], questions_already_open[], claims_to_check[]}, threads_found, absorbed, remaining, questions_filed, claims_to_check, claims_by: model \| mechanical, main_question, verify_job_id?, summary, next}`. The transcript is shelved as it is; the person's questions join the frontier (type person); the assistant's claims are filed as NOTHING; `verify` files one research run over them. Reads ChatGPT and Claude exports, `[{role, content}]` lists, marked-up transcripts. Route `/v1/absorb`. |
| `library_survey` | `op`=survey \| pick \| do; `path` (keeper only) or `url`; `kind?`=repo \| paper \| site \| issues (detected from the thing when not given: a folder or a git url is a repo, a GitHub issues page is issues, a DOI, an arXiv page, a PDF or a document file is a paper, any other url is a site); `name?` (as surveyed, for pick and do); `picks?` (`"1,3"`); `question?` (for do); `patron?` | a thing the person has as a starting point. survey: `{kind, name, origin, title, facts{}, what_it_is, summary_text, claims[], rests_on[], leaves_open[], described_by: model \| mechanical, raw, claim_id, options[] {n, question}, already_open[], summary, next}` — the text shelved (collection repos \| papers \| sites \| issues), one draft claim, the directions on the open questions as type `person`, nothing run. A git url is cloned when git is installed; without git the call says so. pick/do: `{kind, name, origin, claim_id, runs[] {n?, question, job_id \| error}, options[] (still open), summary, next}`. `library_repo` is the same call with kind=repo. |
| `library_items` | one of `path` (keeper only), `url`, `text`; `lens?` (`{item}` marks the item), `as`=frontier \| runs \| none, `title?`, `column?`, `collection`=lists, `limit`=200 (the most looked at per call; the whole list is shelved), `match?` (a text, or several, every one of which an item's line must hold), `sample?` (that many of the matched, at random), `patron?` | a list of things as a starting point: `{title, raw, items_found, taken, remaining, lens, as, items[] {item, note?, held (an entry id or null), held_title?, question, filed?}, held, not_held, questions_filed, questions_already_open, jobs[], summary, next}`. The list is shelved as it is; each item is checked against the shelves; `frontier` files a question per item (type person), `runs` files research runs with `sub_questions` = the items in batches of 8, `none` files nothing. Route `/v1/items`. |
| `library_holdings` | `query`, `limit`=20, `patron?` | what the person's own lists hold — every list shelved through items, a Calibre library among them: `{query, lists_hold, matches[] {item, note?, list, raw}, count, summary}`; every word of the query must appear in an entry (title words, an author, a year); an exact lookup, for telling owned from not owned |
| `library_db` | `op`=list \| schema \| query; `database`; for query: `sql` (one reading statement), or for MongoDB `collection` with `filter` or `pipeline` (JSON); `limit`=100 (at most 500); `patron?` (the library owner only) | the databases the owner gave read-only access to. list: `{databases[] {name, kind, address (password masked), hidden_columns[]}, warning?}`. schema: `{schema}` — tables, columns, row counts, sample rows; hidden columns show no values. query: `{columns[], rows[][], more, saved (a `db://…` locator to cite), text, summary}`. Connections are added from the command line only. A statement that is not a single reading statement is refused with the reason. |
| `library_remove` | `id` (a report `I-…` or a claim `F-…`), `what`=all \| report \| claims (a report only), `dry?`, `patron?` (write) | a report or a claim out of the library for good: `{id, kind, title, what, dry, report_goes, claims_go[] {id, title}, claims_stay[], plan, removed[]?, summary, next}`. all = the report and the claims that are its alone; report = the report only; claims = its claims only, the report's list of claims pruned. A claim another report lists or cites stays. Files are deleted, the index and the changes log updated ("removed"); what the run fetched from the web stays. No undo. |
| `library_check` | one of `path` (keeper only), `url`, `text`; `title?`, `collection`=drafts, `fetch_citations`=true, `verify?`, `patron?` | the person's own draft: `{title, raw, claims_to_check[], questions_filed[], questions_already_open[], citations[] {locator, state: shelved \| already held \| requested \| not read, raw?, problem?}, claims_by, verify_job_id?, summary, next}`. Nothing is filed as a finding. Route `/v1/check`. |
| `library_reading` | one of `path`, `url`, `text`; `title?`, `collection?`, `watch?`, `limit`, `patron?` | a reading list (BibTeX, RIS, CSV, lines): `{title, collection, raw, entries_found, taken, remaining, entries[] {title, note?, locator?, state, raw?, problem?}, shelved, requested, no_locator, titles_without_locator[], watching, summary, next}`. Route `/v1/reading`. |
| `library_questions` | one of `path`, `url`, `text`; `as`=frontier \| runs, `title?`, `limit`, `patron?` | a file of questions: `{title, as, questions_found, taken, remaining, questions[] {question, filed? \| job_id?}, jobs[], questions_filed, questions_already_open, summary, next}`. Route `/v1/questions`. |
| `library_bookmarks` | one of `path`, `url`, `text`; `folder?`, `collection?`, `watch?`, `limit`, `patron?` | a bookmarks export: `{title, collection, raw, bookmarks_found, taken, remaining, bookmarks[] {title, url, folder?, state, raw?, problem?}, shelved, requested, watching, summary, next}`. Route `/v1/bookmarks`. |
| `library_meeting` | one of `path`, `url`, `text`; `title?`, `collection`=meetings, `verify?`, `patron?` | a meeting transcript (WebVTT, "Name: words"): `{title, raw, speakers[], questions_filed[], questions_already_open[], claims_to_check[] ("Name said: …"), decisions[], claims_by, verify_job_id?, summary, next}`. Route `/v1/meeting`. |
| `library_bridges` | `op`=run \| list \| accept \| dismiss \| settings \| measure \| distance (`area`, `other`); for run and settings: `area?`, `sources?`, `reach?`, `strict?`, `via`=terms \| graph \| both, `toward?`, `away?`, `since?`, `propose?`/`per_night?`, `dry?`; for accept and dismiss: `question`; `patron?` | discovery by combination: run → `{areas, candidates, novel, declined[], proposals[] {a, c, a_label, c_label, via[], paths[], from_outside[] {middle, with_a, with_c}, sensor, score, hops, random, question, filed?}, tried_outside[], not_novel[], filed, summary}`; list → `{proposals[] {question, date, found_by}, count}`; accept → `{accepted, job_id}`; measure → `{proposed, kept, dismissed, corroborated, by_settings[]}`; distance → `{hops_on_map, siblings, shared_terms[], paths[], shared_sources, sources_naming_both, nearest_concepts[] {a, c, cosine, folded}, embedder, fold_at, summary}`. A proposal is an open question of type `bridge`, never a claim. Route `/v1/bridges`. |
| `library_frontier` | `op`=list \| add \| next \| later \| park \| unpark \| drop \| tidy, `question?`, `patron?`; for list: `type?`, `show?` (queued \| parked \| all, default all), `report?` (an investigation id or its prefix), `fate?` (kept \| waiting \| disputed \| retired \| none), `who?`, `subject?`, `language?`, `q?`, `hints?` | the queue of open questions: `questions[]` `{date, kind, type, text, parked, position, tonight, asked?, report?, report_title?, report_fate?, perspective?, subjects[], language, similar?, similar_n?, answered?}` in queue order — `report` is the investigation that left the question, `report_fate` what became of it, `perspective` the `[…]` tag it was asked from, `similar` the first question of a group that reads alike, and with `hints` `answered` `{id, title, state}` a claim on the shelves that already answers it; add files one; next and later move one to the head or the tail; park keeps one out of the explorer's reach, unpark returns it to the tail; drop closes one; tidy removes duplicate lines and returns `removed`. A report's leftover questions are filed parked. The housekeeping's explorer takes the queue in order, a few runs a night, related questions sharing a run |
| `library_inbox` | `op`=list \| accept \| dispute \| retire; for list: `report?`, `subject?`, `kind?`, `tier?`, `confidence?`, `writer?`, `state?` (draft \| stale), `language?`, `q?`; for a decision: `ids[]?`, `id?`, `report?` (every waiting claim of one investigation), `why` (dispute), `patron?` | the claims waiting for a decision, oldest first: `items[]` `{id, title, state, stale, kind, tier, confidence, writer, date, subjects[], language, sources, report?, report_title?}`; a decision returns `decided[]` `{id, state, title}`, `decision`, `decided_by`. Accept puts a claim into every answer; dispute files the reason and an open question; retire keeps it on disk and out of every answer (write access) |
| `library_serials` | `op`=list \| add \| every \| park \| unpark \| remove, `name?`, `query?`, `every_days`=7, `patron?` | the searches the housekeeping keeps re-running: `searches[]` `{name, query, every_days, last, parked, due}`, or `{kept, name, query, every_days}`, `{name, every_days}`, `{name, parked}`, `{removed, name}`. A parked search is kept and never due until unparked |
| `library_subjects` | `patron?` | `subjects[]` `{id, label, facet, broader, narrower[], count}` — `facet` is required on every subject |
| `library_status` | `patron?` | `counts` by kind (findings by state), `last_updated`, `version` (the release running) |
...
| `library_job` | `op`=read (default) \| stop \| pause \| resume, `job_id?`, `limit`=20, `cursor?`, `patron?` | `{job}` for one id (`state: queued\|running\|done\|failed\|stopped`, `elapsed_s`, `restarted`, `progress` while a research run runs — `{phase, round, rounds, workers_done, workers_total, turns_used, turns_ceiling, at}`, the phase one of planning, workers, critic, synthesis, cite-check, filing; the last one stays on the finished record —, `result` and `investigation` when finished); without an id a PAGE: `active[]` (always), `finished[]` newest-first, `next_cursor`, `finished_total` — the patron's own and the crews'. The ledger is partitioned (active / month folders / landed markers) so nothing here grows with its size — the listing also carries `paused`; `op=stop {job_id}` returns `{job_id, state: stopped \| stopping}` (a queued run never starts, a running one ends at its next turn); `op=pause` and `op=resume` return `{paused}` (write access) |
...
|---|---|---|
| `POST /v1/{ask\|search\|get\|read\|established\|submit\|frontier\|inbox\|subjects\|status\|changes\|map\|perspectives\|sharpen\|explain\|serials\|request_access\|access\|subscribe\|unsubscribe}` | the tool's arguments | the tool's result (§3) |
